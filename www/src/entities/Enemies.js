...
export function spawnBoss(scene) {
    ...
    scene.physics.add.overlap(scene.boss, scene.mimiProjectiles, (b, proj) => {
        ...
        let baseDmg = 15 + (GameState.brushLevel * 5);
...
export function updateBossAI(scene, time) {
    ...
    let lvl = GameState.level || 1;
    if (!scene.bossState) {
        scene.bossState = 'idle';
        scene.bossStateTime = time + 3000 + Math.random() * 2000;
        scene.lastMineTime = time + 8000;
    }

    if (scene.bossState === 'idle') {
        scene.physics.moveToObject(scene.boss, scene.player, 80 + (lvl * 10));

        // Boss shoot classique
        if (time > scene.lastBossShot) {
            ...
            proj.rotation = Phaser.Math.Angle.Between(scene.boss.x, scene.boss.y, scene.player.x, scene.player.y);
            scene.time.delayedCall(4000, () => { if (proj && proj.active) proj.destroy(); });
            scene.lastBossShot = time + Math.max(800, 2000 - (lvl * 100)); 
        }

        // Dispersion de mines
        if (time > scene.lastMineTime) {
            let m = scene.hazards.create(scene.boss.x, scene.boss.y, 'mine');
            m.hazardType = 'mine';
            m.setDepth(14);
            m.setTint(0xff5555);
        ...
        // Transition vers une attaque spéciale
        if (time > scene.bossStateTime) {
            if (Math.random() > 0.5) {
                scene.bossState = 'prepare_dash';
                scene.bossStateTime = time + 1000;
                scene.boss.setTint(0xff0000);
                scene.boss.body.setVelocity(0, 0);
                scene.cameras.main.shake(1000, 0.01);
            } else {
                scene.bossState = 'nova';
                scene.bossStateTime = time + 1000;
                scene.boss.body.setVelocity(0, 0);
                scene.boss.setTint(0xff00ff);
            }
        }
    } else if (scene.bossState === 'prepare_dash') {
        if (time > scene.bossStateTime) {
            scene.bossState = 'dashing';
            scene.bossStateTime = time + 1500;
            let angle = Phaser.Math.Angle.Between(scene.boss.x, scene.boss.y, scene.player.x, scene.player.y);
            let speed = 500 + (lvl * 30);
            scene.boss.body.setVelocity(Math.cos(angle) * speed, Math.sin(angle) * speed);
        }
    } else if (scene.bossState === 'dashing') {
        if (time > scene.bossStateTime) {
            scene.bossState = 'idle';
            scene.bossStateTime = time + 3000 + Math.random() * 2000;
            scene.boss.clearTint();
        }
    } else if (scene.bossState === 'nova') {
        if (time > scene.bossStateTime) {
            if (typeof window.playBossShootSound === 'function') window.playBossShootSound();
            for(let i=0; i<8; i++) {
                ...
                proj.setDepth(19);
                proj.rotation = angle;
                let speed = 200 + (lvl * 15);
                scene.physics.velocityFromRotation(angle, speed, proj.body.velocity);
                scene.time.delayedCall(5000, () => { if (proj && proj.active) proj.destroy(); });
            }
            scene.bossState = 'idle';
            scene.bossStateTime = time + 4000 + Math.random() * 2000;
            scene.boss.clearTint();
        }
    }
    ...
    if (time > scene.lastMimiShot && !scene.player.isStunned) {
        ...
        let speedBuff = Math.max(0, (GameState.speedLevel - 1) * 30);
...
export function playerTakeDamage(scene, proj, severe) {
    ...
    if (scene.player.hasPearlShield) {
        ...
        if (scene.pearlShieldGfx) {
            scene.pearlShieldGfx.destroy();
            scene.pearlShieldGfx = null;
        }
        ...
        const expl = particleManager.createEmitter({
            x: scene.player.x, y: scene.player.y, speed: 200, scale: {start:2, end:0}, tint: 0x00ffff, lifespan: 500
        });
        expl.explode(30);
...
export function damageBoss(scene, proj, amount, isCrit = false) {
    if (proj) {
        ...
        const impactParticles = sparkleManager.createEmitter({
            x: impactX, y: impactY,
        ...
        scene.time.delayedCall(1000, () => sparkleManager.destroy());
...
export function defeatBoss(scene) {
    ...
    const explosionManager = scene.add.particles('sparkle');
    const explosion = explosionManager.createEmitter({
        x: scene.boss.x, y: scene.boss.y,
        speed: { min: 400, max: 1200 }, scale: { start: 4, end: 0 },
        lifespan: 3000, blendMode: 'ADD'
    });
    explosion.explode(200);
    scene.time.delayedCall(4000, () => explosionManager.destroy());
